# -*- mode: python ; coding: utf-8 -*-
"""
PyInstaller spec file for video backend executable.
This creates a standalone backend with cv2 bundled.
"""

a = Analysis(
    ['backend_main.py'],
    pathex=[],
    binaries=[],
    datas=[
        ('assets/TrapperAI-v02.2024-YOLOv8-m.pt', '.')
    ],
    hiddenimports=[
        'video_backend',  # Explicitly include the backend module
        'uvicorn.logging',
        'uvicorn.loops',
        'uvicorn.loops.auto',
        'uvicorn.protocols',
        'uvicorn.protocols.http',
        'uvicorn.protocols.http.auto',
        'uvicorn.protocols.websockets',
        'uvicorn.protocols.websockets.auto',
        'uvicorn.lifespan',
        'uvicorn.lifespan.on',
        # Add-ins for Uvicorn stability
        'httptools',
        'uvloop',
        'websockets',
        # Add-ins for core scientific packages
        'cv2',
        'torch',
    ],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[],
    noarchive=False,
    optimize=0,
)

pyz = PYZ(a.pure)

exe = EXE(
    pyz,
    a.scripts,
    a.binaries,
    a.datas,
    [],
    name='trailcam_backend',
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=True,
    upx_exclude=[],
    runtime_tmpdir=None,
    console=False,  # Background process, logging handled internally
    disable_windowed_traceback=False,
    argv_emulation=False,
    target_arch=None,
    codesign_identity=None,
    entitlements_file=None,
    mac_sdk_roots=["/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk"],
)
